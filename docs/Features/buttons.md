...
| `apps/web/app/register/youngsters/page.tsx` | `Register Youngster` | `POST /api/v1/auth/register/youngsters` (`GET /auth/register/schools` for select preload) | One-flow youngster + parent registration/linking with required registrant type (`Youngster/Parent/Teacher`); if `Teacher`, `teacherName` is required (max 50). | Creates/links `users.id`, `children.id`, `parents.id`; links parent-child relation row; writes `children.registration_actor_type` and `children.registration_actor_teacher_name` (teacher flow). |
