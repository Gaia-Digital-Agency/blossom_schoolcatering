...
function normalizeLegacyGrade(raw: string) {
  ...
  if (/^KINDER(GARTEN)?$/i.test(value)) return 'Kinder';
...
export function progressGradeByYears(baseGrade?: string | null, yearsElapsed = 0) {
  ...
  const startIndex = GRADE_INDEX.get(normalized.toUpperCase());
  if (startIndex === undefined) return normalized;
  const nextIndex = Math.min(startIndex + Math.max(0, yearsElapsed), GRADE_OPTIONS.length - 1);
