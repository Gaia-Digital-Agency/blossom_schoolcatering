...
Current remaining `sqlLiteral(...)` usage count in `core.service.ts`:
- `214`

This means migration has started in controlled batches but is not yet complete.
...
- SQL injection surface in `core.service.ts` reduced through first two parameterization batches.

Still pending:
- Complete remaining `214` `sqlLiteral(...)` usages in `core.service.ts` to finish full migration.
